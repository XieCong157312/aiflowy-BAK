...
## v2.1.3 2026-06-12
- 新增: 新增工作流与执行记录对外开放API控制器，完善对外开放API控制器与安全能力
- 新增: 新增系统环境变量管理功能
- 新增: 开发可对外发布的Web SDK工具包
- 新增: 支持用户中心接入与嵌入功能
- 新增: 支持通过URL参数同步主题、链接参数、智能体选择、会话及智能体信息
- 新增: 知识库新增AI自动优化标题与描述能力
- 新增: 知识库数据提供者支持加载子知识库内容
- 新增: 新增智能体集成与嵌入独立文档页面，配套集成文档上传智能体发布示意图
- 优化: 重构InputStreamFile实现，支持懒加载并优化文件解析逻辑；统一工作流参数类型为小写
- 优化: 页面布局与样式细节优化，格式化VitePress配置文件
- 优化: 升级项目全部依赖，升级@tinyflow-ai/vue至1.3.4版本
- 优化: 项目仓库迁移至GitHub，更新包发布配置
- 修复: 去除分类数据库分页重复查询逻辑
- 修复: 校验公开对话接口消息序列合法性
- 修复: 完善模型校验与配置检查逻辑
- 修复: 保证历史聊天记录顺序稳定，空列表安全返回
- 修复: 限制任务工具仅可访问允许的包路径
- 修复: 修改智能体消息表内容字段为长文本类型，设置智能体最大回复长度默认值20000
- 修复: 修复系统潜在安全漏洞
- 修复: 详情查询接口增加资源归属权限校验
- 修复: 修改知识库父ID字段为无符号大整数类型
- 文档: 文档内将「聊天助手」统一重命名为「智能体」
- 文档: 更新产品文档侧边栏，新增功能配套文档，补充知识库文档并调整侧边栏链接
- 文档: 替换文档内bots.png图片资源
- 文档: 更新Docker部署指引，调整快速上手导航栏并补充各类部署文档
- 文档: 完善WebSDK配套文档、侧边栏入口，优化WebSDK图片样式并更新文档内容
- 文档: 文档新增站内本地搜索功能，优化页面头部布局
- 文档: 更新知识库文档链接引用

## v2.1.2 2026-04-24
- 新增: 新增 Wiki 模块，支持机器人绑定与集成能力
- 新增: 新增 Iframe 对话视图，支持外部路由访问
- 新增: 实现对话侧边栏可折叠功能，优化页面整体布局
- 新增: 新增 OCR模型 管理展示页面，增加 OCR 模型配置项
- 优化: 调整对话页面布局结构，优化侧边栏交互体验
- 修复: 优化数 bot 数据删除联动逻辑，删除 bot 时同步清理关联数据表
- 修复: 完善级联删除规则，删除知识库、工作流、插件等资源时同步清理机器人关联数据
- 修复: 修复 ES 搜索引擎知识库未分库的问题
- 修复: 修复 Lucene 搜索引擎分库存储能力
- 修复: bot存储预设问题bug

## v2.1.1 2026-04-10
- 新增：删除知识库、向量数据库数据及搜索引擎数据
- 新增：向量数据库独立提取模块，便于使用与验证；知识库检索采用加权召回方式
- 新增：对话工具调用次数上限限制
- 优化：更新对话框“预设问题”为换行显示
- 优化：更新选择资源对话框样式
- 优化：更新用户中心菜单标题
- 优化：更新管理后台 bot 页面标题
- 优化：发送消息后隐藏问题预设
- 优化：更新依赖 agents-flex 和 tinyflow-java 版本
- 修复：修复工具循环调用问题
- 修复：修复 AI 聊天流异常处理
- 修复：删除向量数据库内容时使用 delete 方法
...
- 新增：工具调用的参数和结果展示功能
- 新增：支持查询知识库并筛选元数据
- 优化：更新 tinyflow-java 依赖至 rc.7 版本
- 优化：更新 agents-flex 依赖版本（从 rc.7 升级至 2.0.2）
- 优化：启动聊天后隐藏加载状态
- 优化：修改 tb_ot_mcp 表中 mcp_tool_description 字段类型为 text
- 修复：聊天助手对话空白消息问题
- 修复：工具查询挂载相关问题
- 修复：聊天框发送消息时头部弹窗问题
- 修复：函数调用回调参数空指针问题
- 文档：更新 MCP 文档，补充详细配置指南

## v2.0.9 2026-02-13
- 新增：新增聊天历史页面
- 新增：查询会话接口返回助手信息
- 新增：聊天助手增加附件上传功能
- 优化：升级 agents-flex 依赖至 2.0.0-rc.7 版本
- 优化：向量模型错误提示
- 优化：优化知识库内部配置
- 优化：用户中心会话查询接口返回助手信息
- 优化：聊天记录接口发布
- 优化：更新 @tinyflow-ai/vue 依赖
- 优化：根据 suspendForParameters 长度条件渲染 ElFormItem
- 优化：更新 tinyflow-java 依赖至 rc.6 版本
- 优化：发送问题消息后清空文件列表
- 优化：移除用户文件识别后立即删除文件
- 优化：状态变更时停止确认按钮加载
- 优化：工作流步骤中显示流程错误信息
- 优化：简化工作流状态获取中的节点处理逻辑
- 优化：增强工作流节点排序以支持嵌套结构
- 修复：处理文件地址中包含中文导致的 URL 错误
- 修复：执行记录的跳转链接
- 修复：支持 macOS 上 Cmd+S 保存快捷键

## v2.0.8 2026-02-06
- 新增: 工作流模块支持上传JSON文件导入工作流
- 新增: 支持第三方通过访问令牌调用聊天助手接口
- 新增: 聊天助手工具调用新增参数展示功能
- 新增: 聊天助手新增工具调用展示功能
- 新增: 聊天助手新增排序功能
- 新增: 知识库新增Markdown标题层级拆分器
- 优化: 优化CronPicker对话框的使用体验
- 优化: 对代码进行注释规范
- 优化: 降级vue-element-plus-x依赖版本，恢复HTML预览功能
- 优化: 解决聊天助手选择工具页面复选框显示不清晰问题，支持点击整行选中
- 优化: 调整聊天气泡列表头部宽度样式
- 优化: 优化潜在的空指针问题，返回友好的提示信息
- 优化: 统一用户中心对话界面与后端接口代码逻辑，提取公共方法
- 优化: 角色添加对话框启用menuCheckStrictly选项
- 优化: 为工作流添加confirmItem本地化配置及默认标签
- 优化: 递归更新嵌套链状态中的节点状态
- 优化: 更新banner展示内容
- 优化: 更新mybatis-flex、tinyflow-java依赖版本
- 修复: 优化ollama接口的错误返回逻辑
- 修复: 修复聊天模块最新消息更新异常的问题
- 修复: 修复SyncApis主方法运行时生成接口的报错问题
- 修复: 修正工作流节点状态判断条件，将6改为5
- 修复: 修复tinyflow嵌套链状态下节点状态更新不彻底的问题
- 文档: 新增用户反馈相关文档
- 文档: 新增模型管理相关文档
- 文档: 新增MCP商店相关文档
- 文档: 新增聊天助手挂载MCP相关文档
- 文档: 新增数据标注、数据集、租户、系统设置、敏感词、用户通知相关文档
- 文档: 新增MCP相关基础文档
- 文档: 新增聊天助手相关文档
- 文档: 新增机器人相关文档
- 文档: 新增access token使用与管理相关文档
- 文档: 新增知识库相关文档
- 文档: 新增国际化、任务相关文档
- 文档: 新增资源、数据中心相关文档
- 文档: 新增用户、部门、岗位、角色、菜单、外观相关文档
- 文档: 新增OAuth认证的完整配置指南
- 文档: 更新vitepress侧边栏链接，新增告警管理相关文档
- 文档: 新增菜单配置相关文档
- 文档: 为文档中心新增菜单配置
- 文档: 向侧边栏中添加资源和数据中心相关板块
- 文档: 替换JavaScript执行相关示例
- 文档: 更新工作流节点上下文、条件执行、循环执行相关文档
- 文档: 新增工作流快速入门指南
- 文档: 更新文档中心的结构与内容
- 文档: 新增版本变更记录文档
